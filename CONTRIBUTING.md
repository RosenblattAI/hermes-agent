...
| **Node.js 20+** | Optional — needed for browser tools, pre-commit hooks, and WhatsApp bridge (matches root `package.json` engines) |
...
# Docs site + workspace dependencies, plus pre-commit hooks (husky)
npm install
```
...
# Workspace / docs dependencies, plus pre-commit hooks (husky)
npm install
```
