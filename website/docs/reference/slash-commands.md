...
| `/btw <question>` | Ask an ephemeral side question that uses the current session's context but runs without tools — the answer is reported back and nothing is written to the session or triggered. |
...
| `/copilot_remote [launch\|list\|show]` | Launch, list, or inspect tracked Copilot remote jobs. See [Copilot Remote Jobs](/user-guide/features/copilot-remote). |
...
- `/status`, `/egress`, `/version`, `/whoami`, `/background`, `/btw`, `/copilot_remote`, `/queue`, `/steer`, `/voice`, `/reload-mcp`, `/reload-skills`, `/rollback`, `/diff`, `/debug`, `/fast`, `/approvals`, `/footer`, `/curator`, `/kanban`, `/topup`, `/suggestions`, `/blueprint`, `/learn`, `/init`, `/sessions`, and `/yolo` work in **both** the CLI and the messaging gateway.
