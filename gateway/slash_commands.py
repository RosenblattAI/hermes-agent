...
class GatewaySlashCommandsMixin:
    ...
    # === ROSENBLATT PATCH START: /btw ephemeral side question command ===
    # Reason: Rosenblatt-specific command for ephemeral side questions.
    # Re-landed into GatewaySlashCommandsMixin during the 2026-08-24 upstream
    # sync (handler bodies moved from run.py to this mixin upstream).
    async def _handle_btw_command(self, event: MessageEvent) -> str:
        ...
        return f'\U0001f4ac /btw: "{preview}"\nReply will appear here shortly.'
    ...
    async def _run_btw_task(
        ...
        from gateway.run import _AGENT_PENDING_SENTINEL, _load_gateway_config, _platform_config_key
        ...
        try:
            ...
            def run_sync():
                ...
                try:
                    ...
                    _platform_label = source.platform.value if hasattr(source.platform, "value") else source.platform
                    _txn_name = f"gateway.btw {_platform_label}"
                    with start_transaction(op="hermes.chat", name=_txn_name) as txn:
                        if txn is not None:
                            txn.set_tag("session_id", task_id or "")
                            txn.set_tag("platform", str(_platform_label))
            ...
            preview = question[:60] + ("..." if len(question) > 60 else "")
            header = f'\U0001f4ac /btw: "{preview}"\n\n'
