...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    """
    # [rosenblatt] Sentry span for individual tool execution (concurrent path).
    # Wraps the full dispatch so tool latency + input/output land on the span.
    try:
    ...
    # === ROSENBLATT PATCH START: Sentry span for tool execution ===
    # Reason: Wrap tool execution in Sentry span for observability
    # Upstream: internal-only (Rosenblatt-specific instrumentation)
    with _sentry_tool_span(
        ...
        if _tool_span is not None:
            try:
                from hermes_sentry import sanitize_observability_text as _sot
                _tool_span.set_data("output", _sot(_result, limit=1000))
            except Exception:
                pass
        return _result
