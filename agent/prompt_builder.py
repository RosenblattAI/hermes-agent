...
COPILOT_REMOTE_DELEGATION_GUIDANCE = (
    ...
    "When the copilot_remote tool is available, treat it as an opt-in "
    "execution path, not the default for requests merely about implementation.\n"
    "Before launching, classify the user's request as discussion or execution.\n"
    "Discussion requests ask Hermes to explain, compare, answer when/should "
    "questions, plan, or review an idea about implementation without asking "
    "Hermes to perform repository work. Do not launch copilot_remote for "
    "discussion requests; answer them directly with normal tools.\n"
    "Execution requests explicitly ask Hermes to write, create, build, edit, "
    "fix, implement, refactor, test, open a PR, update a file, or otherwise "
    "change code, websites, documentation files, configuration, scripts, or "
    "repository files. Do not infer execution intent just because the request "
    "mentions code or implementation details.\n"
    "For these explicit execution requests, call copilot_remote(action=\"launch\") "
