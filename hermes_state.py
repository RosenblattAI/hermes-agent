    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 10:
                # v10: add copilot_jobs table for
            ...
            if current_version < 11:
                # v11: simplify copilot_jobs — drop old columns and events table.
            ...
            if current_version < 12:
                # v12: drop copilot_session_id — the job id IS the session UUID.
                cursor.executescript("""
                    CREATE TABLE IF NOT EXISTS copilot_jobs_v12 (
                        id TEXT PRIMARY KEY,
                        hermes_session_id TEXT REFERENCES sessions(id),
                        repo_slug TEXT NOT NULL,
                        repo_path TEXT NOT NULL,
                    ...
                    INSERT OR IGNORE INTO copilot_jobs_v12
                    ...
                    ALTER TABLE copilot_jobs_v12 RENAME TO copilot_jobs;
                    CREATE INDEX IF NOT EXISTS idx_copilot_jobs_state
                        ON copilot_jobs(state);
                    CREATE INDEX IF NOT EXISTS idx_copilot_jobs_repo
                        ON copilot_jobs(repo_slug, state);
            ...
            if current_version < 13:
                # v13: add remote-session tracing, Jira linkage, wall-clock
                # timeout, and retry provenance to copilot_jobs.
                # Add copilot_job_hooks for merge-gate and post-task callbacks.
                new_job_columns = [
                    ("connect_id", "TEXT"),
                    ("jira_issue_key", "TEXT"),
                    ("deadline_at", "REAL"),
                ...
                for col_name, col_type in new_job_columns:
                    try:
                        safe = col_name.replace('"', '""')
                        cursor.execute(
                            f'ALTER TABLE copilot_jobs ADD COLUMN "{safe}" {col_type}'
                        )
                    except sqlite3.OperationalError:
                        pass  # Column already exists (idempotent)
                # Partial index for fast deadline sweep (only possible in new DBs;
                # older SQLite may ignore the WHERE clause but still create the index).
                try:
                    cursor.execute(
