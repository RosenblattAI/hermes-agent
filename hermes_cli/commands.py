...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    # === ROSENBLATT PATCH START: /btw is a distinct Rosenblatt feature ===
    # (ephemeral, no tools, session-context answer) — not an alias of
    # /background, which runs a full background prompt with tools.
    # Upstream coincidentally used "btw" as a /background alias; we keep
    # /btw as its own command and drop the alias above.
    CommandDef("btw", "Ephemeral side question using session context (no tools, not persisted)", "Session",
               gateway_only=True, args_hint="<question>", busy_policy="dispatch"),
